...
int main(void)
{
    ...
    int opcao = 1;
    setsockopt(sockid2, SOL_SOCKET, SO_REUSEADDR, &opcao, sizeof(int));

    if (bind(sockid2, (struct sockaddr *)&sock, sizeof(sock)) < 0)
    {
    ...
    while (1)
    {
        ...
        newSocket_1 = accept(sockid2, (struct sockaddr *)&cliente1, (socklen_t *)&c1);
        if (newSocket_1 < 0)
        {
    ...
    printf("%s\n", buffer_recv);
    bzero(buffer_recv, sizeof(buffer_recv));
    send(newSocket_1, "ok", strlen("ok"), 0);
    //close(sockid2);
    //exit(0);
    }
}