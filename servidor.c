...
void connect_client2(char *ip, char *port, Pergunta pcliente1[3], int socket_cliente1)
{
    struct sockaddr_in sock;
    int con, sockid2,pontos_cliente1=0,pontos_cliente2 = 0;
    ...
    con = connect(sockid2, (struct sockaddr *)&sock, sizeof(sock));
    if (con < 0)
    {
    ...
    else
    {
        ...
        send(sockid2, msg, strlen(msg), 0);
        recv(sockid2, con_info, sizeof(con_info), 0);
        ...
        if (con_info[0] == 's')
        {
            ...
            if (strcmp(msg, "ready") == 0)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    printf("\nMensagem: %s \n", msg);
                    send(sockid2, msg, strlen(msg), 0);
                    recv(sockid2, msg_received, sizeof(msg_received), 0);
                ...
                //enviando a struct do cliente 2 pro cliente 1:
                sprintf(msg, "5");
                send(socket_cliente1, msg, strlen(msg), 0);
                bzero(msg, sizeof(msg));
                recv(socket_cliente1, msg_received, sizeof(msg_received), 0);
                bzero(msg_received, sizeof(msg_received));
                sprintf(msg, "Pergunta 1: %s \na) %s \nb) %s \nc) %s \nd) %s \nDigite a sua resposta (a, b, c ou d):", p[0].pergunta, p[0].resp_1, p[0].resp_2, p[0].resp_3, p[0].resp_4);
                send(socket_cliente1, msg, strlen(msg), 0);
                recv(socket_cliente1, msg_received, sizeof(msg_received), 0);
                if (msg_received[0] == p[0].resp_certa[0])
                {         
                    bzero(msg_received, sizeof(msg_received));       
                    bzero(msg, sizeof(msg));
                ...
                sprintf(msg, "Pergunta 2: %s \na) %s \nb) %s \nc) %s \nd) %s \nDigite a sua resposta (a, b, c ou d): ", p[1].pergunta, p[1].resp_1, p[1].resp_2, p[1].resp_3, p[1].resp_4);
                send(socket_cliente1, msg, strlen(msg), 0);
                bzero(msg, sizeof(msg));
                bzero(msg_received, sizeof(msg_received));
                recv(socket_cliente1, msg_received, sizeof(msg_received), 0);
                printf("\nResposta do cliente 1: %s\n", msg_received);
                if (msg_received[0] == p[1].resp_certa[0])
                {
                    bzero(msg_received, sizeof(msg_received));       
                    bzero(msg, sizeof(msg));
                    pontos_cliente1 = pontos_cliente1 + 50;
                ...
                sprintf(msg, "Pergunta 3: %s \na) %s \nb) %s \nc) %s \nd) %s \nDigite a sua resposta (a, b, c ou d): ", p[2].pergunta, p[2].resp_1, p[2].resp_2, p[2].resp_3, p[2].resp_4);
                send(socket_cliente1, msg, strlen(msg), 0);
                bzero(msg, sizeof(msg));
                bzero(msg_received, sizeof(msg_received));
                recv(socket_cliente1, msg_received, sizeof(msg_received), 0);
                printf("\nResposta do cliente 1: %s\n", msg_received);
                if (msg_received[0] == p[2].resp_certa[0])
                {
                    bzero(msg_received, sizeof(msg_received));       
                    bzero(msg, sizeof(msg));
                    pontos_cliente1 = pontos_cliente1 + 50;
            ...
            //comparando as pontuações e mandando quem foi o ganhador pra cada cliente
            if(pontos_cliente1>pontos_cliente2)
            {
                strcpy(msg,"Cliente 1 venceu!\n");
                send(socket_cliente1, msg, strlen(msg), 0);
                recv(socket_cliente1, msg_received, sizeof(msg_received), 0);
                bzero(msg_received, sizeof(msg_received));
                send(sockid2, msg, strlen(msg), 0);
            ...
            else if(pontos_cliente1<pontos_cliente2)
            {
                strcpy(msg,"Cliente 2 venceu!\n");
                send(socket_cliente1, msg, strlen(msg), 0);
                recv(socket_cliente1, msg_received, sizeof(msg_received), 0);
                bzero(msg_received, sizeof(msg_received));
                send(sockid2, msg, strlen(msg), 0);
    ...
    while (1)
    {
        ...
        if (pid == 0)
        {
            ...
            if (buffer_receive[0] == 's')
            {
                ...
                //printf("\nChamando conexão");
                connect_client2(ip_cliente2, port_cliente2, p, socket_cliente1); // <---- parei aqui
                //printf("\nConexão chamada %d\n", a);
                exit(0);
                //  ------------------------------
            }
